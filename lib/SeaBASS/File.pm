...
/missing or /below_detection_limit headers, they are set to undef when they 
are retrieved.
...
sub write {
	...
	if ( !$error ) {
		...
		my $bdl     = ( exists( $self->{'below_detection_limit'} ) ? $self->{'below_detection_limit'} : $DEFAULT_MISSING );
		...
		if ( $self->{'options'}{'preserve_header'} ) {
			...
			if ($missing eq $bdl || $self->{'options'}{'missing_data_to_undef'}){
				@headers_to_print = grep(!/below_detection_limit/i, @ALL_HEADERS);
			} else {
	...
	while ( my ( $k, $v ) = $iterator->() ) {
		if ( $self->{'options'}{'missing_data_to_undef'} ) {
			if ( $self->{'missing_is_number'} && looks_like_number($v) && $v == $self->{'missing'} ) {
				$ret{$k} = undef;
			} elsif ( $self->{'bdl_is_number'} && looks_like_number($v) && $v == $self->{'below_detection_limit'} ){
				$ret{$k} = undef;
            } elsif ($v eq $self->{'missing'} || $v eq $self->{'below_detection_limit'}) {
				$ret{$k} = undef;
            } else {
				$ret{$k} = $v;
            }
